...
type
  ...
  TAmItemsPrm = record
     ...
     case TAmItemsEnum of
         ...
         // lcChildItemAdding:(ItemChildAdding:TObject;CanAdd:boolean);
  ...
  ['{1E5C41F9-031B-4690-878B-0B002B5D8470}']
   ...
   //protected
      function  CanChangedList(Now:IAmListItemOwner; var New:IAmListItemOwner):boolean;
  ...
  AmItemHelp =class
   ...
   public
    ...
    class function  ListSet(Item:IAmItem;var Field:IAmListItemOwnerPointer; NewValue: IAmListItemOwner):boolean;
  ...
  TAmItemObject =class (TAmInterfacedObject,IAmItem)
    ...
    protected
      ...
      function  ItemCanChangedList(Now:IAmListItemOwner; var New:IAmListItemOwner):boolean;virtual;
  ...
  TAmListItemCustom = class abstract (TAmListBaseInterfaced<IAmItem>,IAmListItem,IAmListItemOwner)
   ...
   //  FItemClass:TAmItemClass;
     FList: TList<IAmItem>; // Todo переделать на TList и при добалении _AddRef при удалении _Release
     FIdCounter:Cardinal;
  ...
  TAmListItemColection = class (TAmListItemPers,IAmListColection)
   private
    ...
    //  function IAmListColection.ItemsUnsafe = wcItemsUnsafe;
...
resourcestring
  Rs_TAmItemPersInf_BeforeDestruction  = 'Error TAmItemPersInf.BeforeDestruction  не удалены все ссылки на интерфейс [ "%s" : Лишних ссылок = "%s"]';
  Rs_TAmListItemCustom_SupObjToItem  = 'Error TAmListItemCustom.SupObjToItem объект ["%s"] не поддерживает interface IAmItem';
  Rs_TAmListItemCustom_EmulAdd1  = 'Error TAmListItemCustom.EmulAdd Item = nil';
  Rs_TAmListItemCustom_EmulAdd2  = 'Error TAmListItemCustom.EmulAdd Item.List <> IAmListItemOwner(self)';
  Rs_TAmListItemCustom_EmulRemove  = 'Error TAmListItemCustom.EmulRemove Item = nil';
  Rs_TAmListItemCustom_DublicateItem  = 'Error TAmListItemCustom.%s  IAmItem уже находится в этом листе выполните move exchange или listSet(nil) прежде чем вызывать %s';
...
begin
  ...
  Result:= (Item <> nil) and ( AFieldList <> NewValue) and Item.CanChangedList(AFieldList,NewValue);
  Result:= Result and ( AFieldList <> NewValue);
  if not  Result then  exit;

  Item.Changed(AmItemOperation.ChangeListBefore,0,LPARAM(NewValue));
  try
   // AFieldList:= IAmListItemOwner(Field);
...
function TAmItemComponent.ItemCanChangedList(Now:IAmListItemOwner; var New:IAmListItemOwner): boolean;
begin
   Result:=true;
end;
...
begin
    ...
    if (List <> nil) then
     dec(C);
    if C <> 0 then
     raise Exception.CreateResFmt(@Rs_TAmItemPersInf_BeforeDestruction,[self.ClassName,C.ToString]);
...
function TAmItemObject.ItemCanChangedList(Now:IAmListItemOwner; var New:IAmListItemOwner): boolean;
begin
  Result:=true;
end;
...
procedure TAmListItemCustom.EmulChangeItem(Item: IAmItem; Operation,W,L: integer);
var Prm:TAmItemsPrm;
begin
  ChangeLock;
  try
...
function TAmListItemCustom.ItemsChangedBroadcast(Operation, W, L: integer):integer;
var i:integer;
begin
   Result:=0;
